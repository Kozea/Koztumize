...
class Button(Directive):
    """ReST directive creating a button."""
    ...
    def run(self):
        content = (
            '<input type="button" class="%s" value="%s" onclick="%s"/>' % (
...
def editable(name, rawtext, text, lineno, inliner, options=None, content=None):
    """ReST role creating an editable span."""
...
class Script(Directive):
    """ReST directive creating a script tag in HTML."""
    ...
    def run(self):
        ...
        content = (
            '<script src="%s.js" type="text/javascript"></script>' % path)
        return [docutils.nodes.raw('', content, format='html')]


class JQuery(Directive):
    """ReST directive including JQuery."""
...
class Checkbox(Directive):
    """ReST directive creating a checkbox in HTML."""
    required_arguments = 0
    optional_arguments = 0
    final_argument_whitespace = True
    has_content = False
    ...
    def run(self):
        content = (
            '<input type="checkbox" onChange="this.setAttribute('
